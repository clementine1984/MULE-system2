...
db.startSocketServer(httpServer);
...
function isJSON(req, res, next) {
  ...
  if (!contype || contype.indexOf('application/json') !== 0) return res.send(415);
  next();
}
...
app.get("/user/:id", async function(req, res) {
  try {
    ...
    var result=await User.getJSON({id:req.params.id})
    console.log(result)
...
app.get("/user/:id/settings", async function(req, res) {
  try {
    //assert(req.decoded.id == req.params.id, { "name": "JsonWebTokenError", "message": "Cannot get data for user of different id" });
		...
		var result=await UserSettings.getJSON({id:req.params.id})
...
app.post("/user/:id/settings", async function(req, res) {
  try {
    //assert(req.decoded.id == req.params.id, { "name": "JsonWebTokenError", "message": "Cannot get data for user of different id" });
		...
		var settings=UserSettings.fromJSON(req.body)
...
app.post("/user/:id/consent", async function(req, res) {
  try {
    var consent=Consent.fromJSON(req.body)
...
app.get("/user/:uid/consent", async function(req, res) {
  try {
    var result=await Consent.getJSON({uid:req.params.uid})
    console.log(result);
...
app.get("/getGrades/:qid/:type", async function(req, res) {
  try {
    ...
    if(type==="ca") type="grade" //mapping of names (ca uis stored as grade in DB)

    var results=await Attempt.getGrades(req.params.qid,type);
...
app.get("/users/:course/:consumer", async function(req, res) {
  try {
    var result=await User.getUsers(req.params.course,req.params.consumer)
    console.log(result);
    return res.status(200).json(result);
  }
  catch (err) {
...
app.get("/users/:course", async function(req, res) {
  try {
    var result=await User.getUsers(req.params.course)
    console.log(result);
    return res.status(200).json(result);
  }
  catch (err) {
...
app.post("/setGrade/:id", async function(req, res) {
  try {
    ...
    var result=await Attempt.setGrade(req.params.id,grade,type)
    console.log(result)
    return res.status(200).json(result);
}
  catch (err) {
...
app.post("/setAttempt/:uid", async function(req, res) {
  try {
    var attempt=Attempt.fromJSON(req.body)
...
app.post("/getAttempt/:uid", async function(req, res) {
  try {
    ...
    if(req.body.id!=null)
    {
      var attempt=await Attempt.getJSON({"id":req.body.id});
      assert(attempt.uid===req.body.uid);
      return res.status(200).json(attempt);
    }


    var attempt=await Attempt.getJSON({"uid-qid":[req.body.uid,req.body.qid]});
...
app.post("/getAttemptHistory/:uid", async function(req, res) {
  try {
    ...
    assert(req.params.uid===req.body.uid);
    var attempt=await Attempt.getHistory(req.body.uid,req.body.qid);
...
app.post("/getGrade/:uid", async function(req, res) {
  try {
    ...
    assert(req.params.uid===req.body.uid);
    var attempt=await Attempt.getBest({"uid-qid":[req.body.uid,req.body.qid]});
...
app.post("/getPersonalBest/:uid", async function(req, res) {
  try {
    ...
    assert(req.params.uid===req.body.uid);
    var attempt=await Attempt.getPersonalBest({"uid-qid":[req.body.uid,req.body.qid]});
...
app.post("/external/log", async function(req, res) {
  ...
  try {
    ...
    if(req.signedCookies==null) throw "not permitted"
    if(req.signedCookies['connect.sid']==null) throw "not permitted"
    if(req.signedCookies['connect.sid']==false) throw "not permitted"
