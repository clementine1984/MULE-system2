...
let {store,setRights,setUsers,setWorkbookContents,workbookSelectItem,setHistory,getQid,rewriteHistoryItem,updateGrade}=State
...
let App = null;
{
  ...
  this.state= {
    ...
    //Server methods=====================================================================
    this.server=buildAPI(this.props.proc,["getHistory","updateGrade"])
   ...
   async updateHistory(qid)
   {
     var hist={}

     //if we are overriding the current user, supply that
     var args={qid: qid, uid: null}

     var id=store.getState().workbook.selectedUser.id
     if(id)
     {
       args.uid=id
     }
    
     var history=await this.server.getHistory(args)
     //a little history cleaning/translation

     history.forEach((x)=>
     {
       let t=rewriteHistoryItem(x);
       hist[t.id]=t
     })
     ...
     store.dispatch(setHistory(hist))
      ...
      async updateGrade(res)
      {

        var result=await this.server.updateGrade({id: res.id,type:res.type,grade:res.grade})





        //and update the the state
        store.dispatch(updateGrade(rewriteHistoryItem(result)));
      }
   ...
   componentDidUpdate(prevProps, prevState)
   {
     if(prevProps.selected!=this.props.selected)
     {
       ...
       //otherwise, get the qid of the current node....
       var qid=State.getNode().qid
       if(qid!=null)
       {
           this.updateHistory(qid)
       }
       else {
         store.dispatch(setHistory([]))
       }
    ...
    render() {
      ...
      <React.Fragment>
        ...
        <div className={classes.root} style={{position:"relative",height:"100%",width:"100%",overflow:"auto"}}>



          {(this.props.drawerOpen?(<AppDrawer/>):null)}



      <div  style={{position:"relative",height:"100%",display:"flex",flexDirection:"column"}}>
          <div style={{flex:"0 0 auto"}}>
            <AppBar  launchEditor={this.props.launchEditor}/>
          </div>

          <div style={{display:"relative",flex:" 1",overflowY:"inherit",padding:"8px"}} >
            <DescriptionPanel />
            {((this.props.rights.edit===true)?(<AdminPanel downloadGrades={this.props.downloadGrades} compileWorkbook={this.props.compileWorkbook}/>):null)}
            {((this.props.qid==null)?null:(<HistoryPanel updateGrade={this.updateGrade.bind(this)}  launchEditor={this.props.launchEditor}/>))}
          </div>
  ...
  let mapStateToProps = (state /*, ownProps*/) => {
    return {
      drawerOpen: state.panels.drawer,
      selected: state.workbook.selected,
      contents: state.workbook.contents,
      qid: getQid(state),
          ...
          >
            <tbody>
              ...
              <tr>
                <td>Researchers:</td>
                <td>
                  DDr Kevin Casey, Natalie Culligan, Department of Computer
