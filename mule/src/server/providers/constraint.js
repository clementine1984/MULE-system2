...
function evaltree(context, t) {
    if (t == null)
        return null;
    if (t.left)
        t.left = evaltree(context, t.left);
    if (t.right)
        t.right = evaltree(context, t.right);
    if (t.argument)
        t.argument = evaltree(context, t.argument); //annoyingly for unary args, there is one child under 'argument'. It should be under 'left'
    ...
    if (t.type == "ConditionalExpression") {
        ...
        if (result.type === 'Literal')
            result = result.value;
        if (result === true)
            return evaltree(context, t.consequent);
        else
            return evaltree(context, t.alternate);
    }
    if (t.type == "BinaryExpression") {
        var name = t.operator;
        if (binaryOperators[name] == undefined)
            throw "Unrecognised function:" + name;
        else
            return binaryOperators[name].apply(context, [t.left, t.right]);
    }
    if (t.type == "UnaryExpression") {
        var name = t.operator;
        if (unaryOperators[name] == undefined)
            throw "Unrecognised function:" + name;
        else
            return unaryOperators[name].apply(context, [t.argument]);
    }
    ...
    if (t.type == "MemberExpression") {
        ...
        if (t.object.type == "Identifier") {
            if (t.object.name == "context") {
                ...
                throw "cannot access any objects ouside context";
        }
        else
            e = evaltree(context, t.object);
        if (e == null)
            return null;
        if (e[n] == null)
            return evaltree(context, null);
        else {
...
class ConstraintServiceProvider extends ServiceProvider {
    ...
    evalConstraint(context, constraint, defaultval) {
        if (constraint == null)
            return defaultval;
        if ((constraint == false) || (constraint == true))
            return constraint;
